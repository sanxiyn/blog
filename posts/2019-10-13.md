...
For [IfStatement](https://github.com/estree/estree/blob/master/es5.md#ifstatement) we interpret the condition, and depending on the condition, interpret the body or the else node. In ordert to make empty else interpreting easier, we'll also add a null short-circuit to `interpretStatement`.
